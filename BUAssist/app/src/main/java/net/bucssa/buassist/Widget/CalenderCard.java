...
import net.bucssa.buassist.Util.CalenderUtil;
import net.bucssa.buassist.Util.CustomDate;

/**
 * Created by KimuraShin on 17/8/11.
 */

public class CalenderCard extends View {
    ...
    public CalenderCard(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    public CalenderCard(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public CalenderCard(Context context, OnCellClickListener listener) {
        super(context);
    ...
    private void fillDate() {
        int monthDay = CalenderUtil.getCurrentMonthDay(); // 今天
        int lastMonthDays = CalenderUtil.getMonthDays(mShowDate.year,
                mShowDate.month - 1); // 上个月的天数
        int currentMonthDays = CalenderUtil.getMonthDays(mShowDate.year,
                mShowDate.month); // 当前月的天数
        int firstDayWeek = CalenderUtil.getWeekDayFromDate(mShowDate.year,
                mShowDate.month);
        boolean isCurrentMonth = false;
        if (CalenderUtil.isCurrentMonth(mShowDate)) {
            isCurrentMonth = true;
        }
        int day = 0;
